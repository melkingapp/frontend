...
const MenuButton = ({ icon, label, onClick, open }) => (
    <button
        ...
        tabIndex={open ? 0 : -1}
        onClick={onClick}
        className="flex items-center justify-center gap-1 w-[140px] bg-melkingDarkBlue text-white shadow px-3 py-2 rounded-md text-sm hover:bg-melkingGold hover:text-melkingDarkBlue focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D3B66C] transition whitespace-nowrap"
    >
        {icon}
        <span>{label}</span>
    </button>
        ...
        <div className="fixed bottom-6 left-6 z-50" ref={menuRef}>
            <div className="relative">
                ...
                <button
                    ...
                    className={`w-14 h-14 flex items-center justify-center ${color} text-white rounded-full shadow-lg hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D3B66C] focus-visible:ring-offset-2 transition`}
                >
