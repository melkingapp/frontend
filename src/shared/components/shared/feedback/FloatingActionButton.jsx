...
const MenuButton = ({ icon, label, onClick, tabIndex }) => (
    <button
        onClick={onClick}
        tabIndex={tabIndex}
        className="flex items-center justify-center gap-1 w-[140px] bg-melkingDarkBlue text-white shadow px-3 py-2 rounded-md text-sm hover:bg-melkingGold hover:text-melkingDarkBlue transition whitespace-nowrap focus-visible:ring-2 focus-visible:outline-none"
    >
        {icon}
        <span>{label}</span>
    </button>
        ...
        <div className="fixed bottom-6 left-6 z-50" ref={menuRef}>
            <div className="relative">
                ...
                <button
                    ...
                    className={`w-14 h-14 flex items-center justify-center ${color} text-white rounded-full shadow-lg hover:opacity-90 transition focus-visible:ring-2 focus-visible:outline-none`}
                >
