...
const Menu = ({ items, open, onSelect, id }) => {
    ...
    return (
        <div id={id} role="menu" aria-hidden={!open} className={`absolute left-12 flex flex-col gap-2 items-start transition-all duration-300 ${open ? "opacity-100 scale-100" : "opacity-0 scale-90 pointer-events-none"}`} style={{ top: open ? topOffset : -20 }}>
