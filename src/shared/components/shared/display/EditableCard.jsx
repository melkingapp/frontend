    ...
    return (
        <div className={`mb-6 p-5 rounded-2xl border shadow-md ${colorClass} relative`}>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
                <User /> {title}
                {!isEditing && (
                    <button onClick={() => setIsEditing(true)} className="ml-auto text-gray-500 hover:text-gray-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#D3B66C] rounded" aria-label={`ویرایش ${title}`} title={`ویرایش ${title}`}>
                        <Edit size={16} />
                    </button>
                )}
            </h3>
