...
export default function ManagerHeader({ onOpenSidebar }) {
    ...
    return (
        <header className="h-16 backdrop-blur-md bg-[#1c2e4ef0] text-white px-4 flex items-center justify-between sticky top-0 z-30 shadow-md">
            {/* Mobile Menu Button */}
            <button
                className="lg:hidden text-white hover:text-[#D3B66C] transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
            ...
            <div className="flex items-center gap-4 sm:gap-6">
                {icons.map(({ id, Icon, label, onClick }) => (
                    <button
                        ...
                        className="relative group focus:outline-none focus-visible:ring-2 focus-visible:ring-white rounded-full"
                        aria-label={label}
                        title={label}
                        type="button"
                    >
