...
export default function ManagerHeader({ onOpenSidebar }) {
    ...
    const icons = [
        ...
        { id: "panel", Icon: Home, label: "پنل مدیر", onClick: () => navigate("/manager") },
    ...
    return (
        <header className="h-16 backdrop-blur-md bg-[#1c2e4ef0] text-white px-4 flex items-center justify-between sticky top-0 z-30 shadow-md">
            {/* Mobile Menu Button */}
            <button
                className="lg:hidden text-white hover:text-[#D3B66C] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D3B66C] rounded-md"
            ...
            <div className="flex items-center gap-4 sm:gap-6">
                {icons.map(({ id, Icon, label, onClick }) => (
                    <button
                        ...
                        className="relative group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D3B66C] rounded-full"
                        aria-label={label}
                        type="button"
                    >
