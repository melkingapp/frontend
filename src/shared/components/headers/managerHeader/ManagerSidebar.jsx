        ...
        return navItems.map((item) => {
            ...
            return (
                <div key={item.label} className="space-y-1 w-full">
                    <div
                        className={["flex items-center justify-between w-full group", isCollapsed && !isMobile ? "justify-center" : ""].filter(Boolean).join(" ")}
                    >
                        <Link
                            to={item.to}
                            onClick={() => handleItemClick(item)}
                            className={["flex items-center px-3 py-2 rounded-xl transition-colors duration-300 w-full", isActive
                                    ? "bg-gradient-to-r from-yellow-500 to-yellow-400 text-[#1C2E4E] font-semibold shadow-lg"
                                    : isMobile
                                        ? "hover:bg-gray-100 text-gray-700"
                                        : "hover:bg-white/20 text-white", isCollapsed && !isMobile ? "justify-center" : "gap-3"].filter(Boolean).join(" ")}
                        ...
                        {hasChildren && !isCollapsed && (
                            <button
                                ...
                                className={["mr-2 transition", isMobile
                            ...
                            {item.children.map((child, index) => {
                                ...
                                return (
                                    <Link
                                        key={index}
                                        to={child.to}
                                        className={["block text-sm px-3 py-1.5 rounded-lg transition-colors duration-200 select-none", isChildActive
                                                ? isMobile
