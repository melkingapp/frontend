...
export default function ManagerSidebar({ navItems, sidebarOpen, onCloseSidebar }) {
                ...
                <div key={item.label} className="space-y-1 w-full">
                    <div
                        className={["flex items-center justify-between w-full group",
                            isCollapsed && !isMobile ? "justify-center" : ""].filter(Boolean).join(" ")}
                    >
                        <Link
                            to={item.to}
                            onClick={() => handleItemClick(item)}
                            className={["flex items-center px-3 py-2 rounded-xl transition-colors duration-300 w-full",
                                isActive
                                    ? "bg-gradient-to-r from-yellow-500 to-yellow-400 text-[#1C2E4E] font-semibold shadow-lg"
                                    : isMobile
                                        ? "hover:bg-gray-100 text-gray-700"
                                        : "hover:bg-white/20 text-white",
                                isCollapsed && !isMobile ? "justify-center" : "gap-3"].filter(Boolean).join(" ")}
                            ...
                            <button
                                ...
                                className={["mr-2 transition",
                                    isMobile
                                        ...
                                        : "text-white/80 hover:text-yellow-400"].filter(Boolean).join(" ")}
                    ...
                    {hasChildren && (isOpen || isChildActive) && !isCollapsed && (
                        <div
                            className={["pl-8 pr-2 mt-1 space-y-1 rounded-r-md border-r-4 border-yellow-400"].filter(Boolean).join(" ")}
                        >
                            {item.children.map((child, index) => {
                                ...
                                return (
                                    <Link
                                        key={index}
                                        to={child.to}
                                        className={["block text-sm px-3 py-1.5 rounded-lg transition-colors duration-200 select-none",
                                            isChildActive
                                                ? isMobile
                                                ...
                                                : isMobile
                                                    ? "hover:bg-gray-100 text-gray-700"
                                                    : "hover:bg-white/20 text-white/90"].filter(Boolean).join(" ")}
                                    >
                                        {child.label}
                                    </Link>
                                );
    ...
    const renderBuildingSelector = (isMobile = false) => (
        <div className="relative z-50">
            <button
                ...
                className={["text-sm font-semibold px-3 py-2 rounded-xl flex items-center justify-between w-full transition",
                    isMobile
                        ? "bg-yellow-100 text-yellow-800"
                        : "bg-yellow-100 text-yellow-900"].filter(Boolean).join(" ")}
            >
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside
                className={["hidden md:flex flex-col shadow-lg space-y-3 overflow-y-auto rounded-l-3xl transition-all duration-300 relative",
                    isCollapsed
                        ? "w-20 items-center bg-gradient-to-b from-[#1C2E4E] to-[#12355B] p-3"
                        : "w-64 bg-gradient-to-b from-[#1C2E4E] to-[#12355B] p-5"].filter(Boolean).join(" ")}
            >
                <button
