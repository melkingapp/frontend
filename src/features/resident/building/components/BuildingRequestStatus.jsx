...
export default function BuildingRequestStatus() {
    ...
    const { uniqueRequests, pendingCount } = useMemo(() => {
        // Bolt: Memoized complex array operations to prevent unnecessary recalculations on re-renders
        if (!requests) return { uniqueRequests: [], pendingCount: 0 };
        ...
        const unique = Object.values(requestsByKey).map(group => {
        ...
        return { uniqueRequests: unique, pendingCount: pending };
    }, [requests]);
