...
export default function RegularRequestsManager() {
  ...
  // ⚡ Bolt Optimization: Memoized to prevent O(n) array filtering on every re-render.
  // Impact: Reduces main-thread blocking when unrelated component state changes.
  ...
  // ⚡ Bolt Optimization: Memoized multiple array filters into a single computed object.
  // Impact: Replaces 3 separate O(n) iterations during render with a single O(1) object lookup per render loop.
  const stats = useMemo(() => {
    return {
