...
export default function UnitTransactionsSummary({
  ...
  // Memoize the filtering of shared expenses to avoid unnecessary calculations on re-renders
  const sharedExpensesCount = useMemo(() => {
    if (!unitTransactions) return 0;
    const txs = unitTransactions.invoices || unitTransactions.transactions || EMPTY_ARRAY;
    return txs.filter((tx) => tx.is_shared_expense).length;
  }, [unitTransactions]);
