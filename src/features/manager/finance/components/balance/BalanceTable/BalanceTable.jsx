...
export default function BalanceTable({ transactions, onTransactionClick, isLoading }) {
  ...
  // تفکیک تراکنش‌ها به دارایی و بدهی با استفاده از useMemo برای جلوگیری از رندر مجدد
  const assetsTransactions = useMemo(() =>
    transactions.filter(t => (t.amount || 0) > 0), // واریزی‌ها و درآمدها
    [transactions]
  );

  const liabilitiesTransactions = useMemo(() =>
    transactions.filter(t => (t.amount || 0) < 0), // برداشت‌ها و هزینه‌ها
    [transactions]
  );
