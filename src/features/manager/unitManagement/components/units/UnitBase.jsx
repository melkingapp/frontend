...
export default function UnitBase({ limit, showCreateButton = true, buildingId = null }) {
    ...
    // ⚡ Performance Optimization: Memoize dataSource and displayedUnits to prevent array reallocation
    ...
    // ⚡ Performance Optimization: Memoize unit grouping logic to avoid O(N) calculation on every render
    const processedUnitItems = useMemo(() => {
        ...
        <div className="p-6 bg-white rounded-xl shadow border border-gray-100">
            ...
            {loading && displayedUnits.length === 0 ? (
                ...
                <div className="space-y-4">
                    {processedUnitItems.map((unit, index) => {
                        if (!unit) return null;
                        return (
