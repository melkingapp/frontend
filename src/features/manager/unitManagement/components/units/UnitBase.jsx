...
export default function UnitBase({ limit, showCreateButton = true, buildingId = null }) {
    ...
    const { dataSource, displayedUnits } = useMemo(() => {
        const source = (reduxUnits || []).filter(unit => unit != null);
        const displayed = limit ? source.slice(0, limit) : source;
        return { dataSource: source, displayedUnits: displayed };
    }, [reduxUnits, limit]);

    // OPTIMIZATION: Memoize the grouped units to avoid recalculating on every render
