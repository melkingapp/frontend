...
export default function UnitBase({ limit, showCreateButton = true, buildingId = null }) {
    ...
    // ⚡ Bolt: Memoize dataSource to prevent re-filtering on every render
    const dataSource = useMemo(() => (reduxUnits || []).filter(unit => unit != null), [reduxUnits]);

    // ⚡ Bolt: Memoize displayedUnits to prevent re-slicing
    const displayedUnits = useMemo(() => limit ? dataSource.slice(0, limit) : dataSource, [dataSource, limit]);

    // ⚡ Bolt: Memoize processedUnits to prevent expensive grouping logic on every render
