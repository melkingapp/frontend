...
export default function UnitBase({ limit, showCreateButton = true, buildingId = null }) {
    ...
    // ⚡ Bolt: Memoize dataSource to avoid re-filtering on every render
    const dataSource = useMemo(() => (reduxUnits || []).filter(unit => unit != null), [reduxUnits]);

    // ⚡ Bolt: Memoize displayedUnits to avoid slicing on every render
    ...
    // ⚡ Bolt: Memoize the grouped units calculation to avoid expensive processing on every render
