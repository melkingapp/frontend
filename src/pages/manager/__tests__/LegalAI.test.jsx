...
describe('LegalAI Manager Page Security Fix', () => {
  ...
  it('migrates chats from localStorage to sessionStorage on mount', async () => {
    ...
    localStorageMock['legalAI_chats'] = JSON.stringify(mockChats);

    render(<LegalAI />);

    await waitFor(() => {
      // Check if sessionStorage has the data
      expect(window.sessionStorage.setItem).toHaveBeenCalledWith('legalAI_chats', JSON.stringify(mockChats));
      // Check if localStorage is cleared
      expect(window.localStorage.removeItem).toHaveBeenCalledWith('legalAI_chats');
    });
  });

  it('loads chats from sessionStorage', async () => {
    const mockChats = [{ id: 2, title: 'Session Chat', messages: [] }];
    sessionStorageMock['legalAI_chats'] = JSON.stringify(mockChats);

    render(<LegalAI />);

    await waitFor(() => {
      expect(window.sessionStorage.getItem).toHaveBeenCalledWith('legalAI_chats');
    });
  });
  ...
  it('does NOT write to localStorage (security check)', async () => {
    ...
    // Ensure localStorage.setItem is NOT called for 'legalAI_chats'
    expect(window.localStorage.setItem).not.toHaveBeenCalledWith('legalAI_chats', expect.anything());
  });
});
